...
export function buildAiPrompt(req: AiActionRequest): string {
  ...
  const actionPrompts: Record<string, string> = {
    ...
    analyzeContent: req.contentType === "video"
      ? `${req.images?.length ? `你正在分析一段视频，收到了 ${req.images.length} 张均匀分布的截图。请仔细观察截图中的人物、场景、文字、物品、动作，推断视频的核心内容和主题。\n\n` : ""}根据以下信息，为这段视频生成元数据。只输出 JSON，不要解释。

{
  "title": "视频标题（30字以内，含关键词）",
  "topic": "具体主题/领域",
  "summary": "100字以内的内容摘要",
  "tags": ["标签1", "标签2"],
  "highlights": ["看点1"],
  "style": "knowledge|tutorial|review|vlog|entertainment|news|commentary|interview",
  "audience": "目标受众",
  "partitionSuggestion": "B站分区建议"
}
