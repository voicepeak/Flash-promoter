...
```bash
npm install             # 安装依赖
npm run dev             # 一键启动 API (:3333) + 桌面工作台 (:5173)
```
...
| 命令 | 说明 |
|:---|:---|
| `npm run dev` | 一键启动 API + 桌面工作台 |
| `npm run dev:api` | 单独启动 API 服务 |
| `npm run dev:desktop` | 单独启动桌面工作台 |
...
npm run dev             # Start API (:3333) + desktop (:5173) in one command
```
...
| Command | Description |
|:---|:---|
| `npm run dev` | Start API + desktop together |
| `npm run dev:api` | Start API server only |
| `npm run dev:desktop` | Start desktop workbench only |
