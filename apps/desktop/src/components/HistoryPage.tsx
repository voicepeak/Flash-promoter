...
export function HistoryPage() {
  ...
  const filteredPosts = filter === "all" ? posts : posts.filter((p) => {
    const ct = (p as Record<string, unknown>).contentType as string | undefined;
    return ct === filter;
  });
        ...
        <section className="card">
          ...
          <div className="compact-list">
            {filteredPosts.slice(0, 20).map((post) => {
              const ct = (post as Record<string, unknown>).contentType as string;
              return (
                <button key={post.id} type="button" className={`compact-row compact-button ${post.id === viewPost ? "active" : ""}`} onClick={() => loadPostDrafts(post.id)}>
                  ...
                  <small>{ct === "video" ? "🎬 视频" : "📄 图文"} · {post.status}</small>
                </button>
              );
            })}
        ...
        <section className="card">
          ...
          <div className="compact-list">
            {jobs.slice(0, 20).map((job) => (
              <div key={job.id} className="compact-row"><span>{platformLabels[job.platform]}</span><small>{job.mode} · {job.status}</small></div>
            ))}
      ...
      {viewPost && drafts.length > 0 ? (
        <section className="card" style={{ marginTop: 16 }}>
          ...
          <div className="drafts-preview-grid">
            {drafts.map((d) => (
              <div key={d.id} className="draft-mini-card"><strong>{platformLabels[d.platform]}</strong><p>{d.title}</p><small>{d.userConfirmed ? "已确认" : "未确认"}</small></div>
            ))}
          </div>
        </section>
      ) : null}
